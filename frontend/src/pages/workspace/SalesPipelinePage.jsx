...
import { Loader2, Phone, MessageCircle, MapPin, Globe, Target, RefreshCw, IndianRupee } from 'lucide-react';
            ...
            <div key={stage.id} className="flex-shrink-0 w-72">
              ...
              <div className={`border-x border-b rounded-b-lg ${stage.color} p-2 space-y-2 min-h-[300px]`}>
                ...
                {stageLeads.slice(0, 20).map(lead => (
                  <div key={lead.id} className="bg-white rounded-lg p-3 shadow-sm border border-white hover:shadow-md transition-shadow cursor-pointer" onClick={() => navigate(`/workspace/sales/leads/${lead.id}`)} data-testid={`pipeline-lead-${lead.id}`}>
                    ...
                    {lead.amount > 0 && <div className="text-xs font-semibold text-emerald-600 mt-0.5 flex items-center gap-0.5"><IndianRupee className="w-3 h-3" />{lead.amount?.toLocaleString('en-IN')}</div>}
